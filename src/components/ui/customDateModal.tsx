...
const buildMarkedDates = (startDate: string | null, endDate: string | null): MarkedDates => {
  if (!startDate) return {};
    ...
    >
      ...
      <View className="flex flex-row w-full justify-between items-center pb-3 border-b border-gray-100 mb-3">
        <View className="flex-row items-center gap-2">
          ...
          <Text className="text-lg font-bold text-[#1E293B]">{t("customDateFilterTitle")}</Text>
        </View>
      ...
      <View className="flex-row items-center justify-between gap-2 mb-3 bg-[#F8FAFC] p-2.5 rounded-xl border border-gray-100">
        <View className="flex-1">
          <Text className="text-xs text-gray-500 font-medium">{t("startCustomDateFilter")}</Text>
          <Text className="text-sm font-semibold text-[#1E293B]">{startDate || "-"}</Text>
        </View>
        <Text className="text-gray-400 font-bold">→</Text>
        <View className="flex-1 items-end">
          <Text className="text-xs text-gray-500 font-medium">{t("endCustomDateFilter")}</Text>
      ...
      <View className="flex-row items-center justify-end gap-2 pt-3 border-t border-gray-100 mt-2">
        ...
        >
          <Text className="text-sm font-semibold text-gray-600">{t("reset")}</Text>
        </Pressable>
        ...
        <Pressable
          ...
          className={`px-5 py-2.5 rounded-xl flex-row items-center justify-center ${
            startDate ? "bg-[#20304E] active:bg-[#162238]" : "bg-gray-200 opacity-60"
          }`}
        >
          <Text className={`text-sm font-bold ${startDate ? "text-white" : "text-gray-400"}`}>
            {t("apply")}
          </Text>
        </Pressable>
      </View>
